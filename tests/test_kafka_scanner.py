...
class BaseScannerTest(unittest.TestCase):
    ...
    def _get_scanner_messages(self, client_mock, kafka_consumer_mock,
...
class KafkaScannerTest(BaseScannerTest):
    ...
    def test_kafka_scan(self, client_mock, kafka_consumer_mock, num_partitions=1, expected_batches=1):
        expected_messages = 7
        client_mock.return_value = FakeClient(self.samples, num_partitions)
        _, number_of_batches, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
                nodedupe=True)
    ...
    def test_kafka_scan_partitions(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan(num_partitions=3, expected_batches=1)

    def test_kafka_scan_count(self, client_mock, kafka_consumer_mock, num_partitions=1, expected_batches=1):
        expected_messages = 2
        client_mock.return_value = FakeClient(self.samples, num_partitions)
        _, number_of_batches, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
    ...
    def test_kafka_scan_count_partitions(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_count(num_partitions=3)

    def test_kafka_scan_batchsize(self, client_mock, kafka_consumer_mock, num_partitions=1):
        expected_messages = 7
        expected_batches = 4
        client_mock.return_value = FakeClient(self.samples, num_partitions)
        _, number_of_batches, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
    ...
    def test_kafka_scan_batchsize_partitions(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_batchsize(num_partitions=3)

    def test_kafka_scan_batchsize_count(self, client_mock, kafka_consumer_mock, num_partitions=1):
        expected_messages = 5
        expected_batches = 3
        client_mock.return_value = FakeClient(self.samples, num_partitions)
        _, number_of_batches, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
    ...
    def test_kafka_scan_batchsize_count_partitions(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_batchsize_count(num_partitions=3)

    def test_kafka_scan_batchcount(self, client_mock, kafka_consumer_mock,
         ...
         scanner, number_of_batches, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
    ...
    def test_kafka_scan_batchcount_batches(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_batchcount(batchsize=200, expected_messages=600)

    def test_kafka_scan_batchcount_one_batch(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_batchcount(batchsize=200, batchcount=1, expected_messages=200)

    def test_kafka_scan_batchcount_partitions(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_batchcount(num_partitions=3)

    def test_kafka_scan_batchcount_batches_partitions(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_batchcount(batchsize=200, num_partitions=3, expected_messages=600)

    def test_kafka_scan_dedupe(self, client_mock, kafka_consumer_mock, batchsize=10000):
        msgs = [('AD%.3d' % i, 'body %d' % i) for i in range(1000)] + \
                [('AD%.3d' % i, 'body %dA' % i) for i in range(100, 200)]
        samples = get_kafka_msg_samples(msgs)

        client_mock.return_value = FakeClient(samples, 3, count_variations={0: 2, 1: 3, 2: 2})
        scanner, _, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
                batchsize=batchsize)
    ...
    def test_kafka_scan_dedupe_batches(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_dedupe(batchsize=200)

    def test_kafka_scan_deleted(self, client_mock, kafka_consumer_mock, batchsize=10000):
        msgs = [('AD%.3d' % i, 'body %d' % i) for i in range(1000)] + \
                [('AD%.3d' % i, None) for i in range(100, 200)]
        samples = get_kafka_msg_samples(msgs)

        client_mock.return_value = FakeClient(samples, 3, count_variations={0: 2, 1: 3, 2: 2})
        scanner, _, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock, batchsize=batchsize)
        msgsdict = {m['_key']: m['body'] for m in messages}
    ...
    def test_kafka_scan_deleted_batches(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_deleted(batchsize=200)

    def test_kafka_scan_deleted_before(self, client_mock, kafka_consumer_mock, batchsize=10000):
    ...
    def test_kafka_scan_deleted_before_batches(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_deleted_before(batchsize=200)

    def test_kafka_scan_nodelete(self, client_mock, kafka_consumer_mock):
        msgs = [('AD%.3d' % i, 'body %d' % i) for i in range(1000)] + \
                [('AD%.3d' % i, None) for i in range(100, 200)]
        samples = get_kafka_msg_samples(msgs)
    ...
    def test_kafka_scan_dedupe_many(self, client_mock, kafka_consumer_mock):
    ...
    def test_kafka_scan_lower_offsets(self, client_mock, kafka_consumer_mock, batchsize=10000):
        msgs = [('AD%.3d' % i, 'body %d' % i) for i in range(1000)] + \
                [('AD%.3d' % i, None) for i in range(100, 200)]
        samples = get_kafka_msg_samples(msgs)
    ...
    def test_kafka_scan_lower_offsets_batches(self, client_mock, kafka_consumer_mock):
        self.test_kafka_scan_lower_offsets(batchsize=200)

    def test_encoding(self, client_mock, kafka_consumer_mock):
        msgs = [('AD001', u'hol\xc3\xa1'.encode('latin1'))]
        samples = get_kafka_msg_samples(msgs)
        client_mock.return_value = FakeClient(samples, 1)
        _, _, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
            encoding='latin1')
        self.assertEqual(messages[0]['body'], u'hol\xc3\xa1')

    def test_wrong_encoding(self, client_mock, kafka_consumer_mock):
        msgs = [('AD001', '>\xc4\xee')]
        samples = get_kafka_msg_samples(msgs)
        client_mock.return_value = FakeClient(samples, 1)
        _, _, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock)
...
class KafkaScannerOverrideTest(BaseScannerTest):
    ...
    def test_process_record(self, client_mock, kafka_consumer_mock):
        msgs = [('AD%.3d' % i, 'body %d' % i) for i in range(1000)] + \
                [('AD%.3d' % i, None) for i in range(100, 200)]
        samples = get_kafka_msg_samples(msgs)
...
class KafkaScannerDirectTest(BaseScannerTest):
    ...
    def test_kafka_scan_batch(self, client_mock, kafka_consumer_mock):
    ...
    def test_kafka_scan_batches_batchcount(self, client_mock, kafka_consumer_mock, batchsize=100, batchcount=3):
        client_mock.return_value = FakeClient(self.samples, 3, count_variations={0: 2, 1: 3, 2: 2})
        _, number_of_batches, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
    ...
    def test_kafka_starting_offsets(self, client_mock, kafka_consumer_mock):
        client_mock.return_value = FakeClient(self.samples, 3, count_variations={0: 2, 1: 3, 2: 2})
        _, number_of_batches, messages = self._get_scanner_messages(client_mock, kafka_consumer_mock,
...
class KafkaScannerDirectResumeTest(BaseScannerTest):
    ...
    def test_kafka_scan_resume_simple_partition(self, client_mock, kafka_consumer_mock, batchsize=100):
    ...
    def test_kafka_scan_resume_simple_partition_after_fail(self, client_mock, kafka_consumer_mock, batchsize=100):
...
#     def test_kafka_scan_resume(self, client_mock, batchsize=100):
#
...
#         sum_msgkeys = 0
#         for batchcount in (2, 2, 4, 2):
#             _, number_of_batches, messages = self._get_scanner_messages(client_mock,
